...
| # | 항목 | 우선순위 | 복잡도 | 상태 | 관련 컴포넌트 | 비고 |
|---|------|---------|--------|------|--------------|------|
| P2-01 | **SCA — 의존성 파일 스캔 및 CVE 매핑**<br>• 대상: pom.xml, build.gradle, package.json, requirements.txt<br>• 라이브러리 버전 추출 → NVD/OSV CVE DB 조회<br>• CVSS 점수 및 영향 버전 범위 매핑<br>• 결과를 공통 스키마(`source_tool=SCA`)로 출력<br>• ✅ `scan_sca.py` v1.0 구현: dependency-check CLI 연동 + 기존 리포트 재사용 + OSV.dev fallback | 🔴 | L | ✅ 완료 | `tools/scripts/scan_sca.py` | 2026-03-19 |
| P2-02 | **SCA — CVE Exploit/PoC 악용 가능성 자동 분석**<br>• CISA KEV 조회 → actively exploited CVE 즉시 식별<br>• 소스코드 관련성 분석: groupId/artifactId grep (실제사용/간접사용/미확인)<br>• PoC 자동 생성: Log4Shell/Spring4Shell/역직렬화/SSRF/인가우회/PathTraversal 템플릿<br>• AI 문맥 판별: 소스코드에서 취약 API 직접 호출 여부 확인<br>• ✅ `scan_sca.py` v1.0 P2-02 기능 포함 구현 완료 | 🟡 | L | ✅ 완료 | `tools/scripts/scan_sca.py` | 2026-03-19 |
