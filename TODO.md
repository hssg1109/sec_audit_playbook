...
| T-09 | **Bitbucket 소스코드 자동 다운로드 → testbed 진단 파이프라인**<br>• `fetch_bitbucket.py` 구현 완료 (PowerShell 경유 사내망 접근)<br>• `CUSTOMER_BB_TOKEN` 계정 단위 토큰 발급 완료 → 300+ 프로젝트 접근 가능 확인<br>• 기능: --project/--repo/--branch/--shallow/--list-projects/--list-repos/--dry-run<br>• 결과: `state/<prefix>_fetch_manifest.json`<br>• T-10 OCB 정기진단 파이프라인과 연계 완료 | 🔴 | M | ✅ 완료 | `tools/scripts/fetch_bitbucket.py`<br>`.env` CUSTOMER_BB_TOKEN | 2026-03-13 | 2026-03-25 | `python3 fetch_bitbucket.py --project <KEY> --shallow` |
| T-10 | **OCB 서비스군 repo별 정기진단 (비전금법 대상)**<br>• 대상: 비전금법 OCB 서비스 — 월별 Fortify 진단 이력 참조 (pageId=728911610)<br>• 진단 방식: repo별 `/sec-audit-static` 전체 워크플로 실행 → 결과 Confluence 게시<br>• 완료 조건: Phase 1~5 전체 + Confluence 게시 + FP/FN 검증<br><br>**[도구 고도화 테스트베드 — 정식 진단 아님]**<br>• 테스트29~31 (OCBWEBVIEW/ocb-webview-api, ocb-service-frontend, ocb-webview-frontend): 스캐너 개선용<br>• 테스트32~34 (OCBWEBVIEW/ocb-community-api, ocb-community-ssr, ocb-ogeul-admin-frontend): 스캐너 개선용<br>• 테스트35~40 (GWS/oki-*, OB/cashbagmall·front_resource·ob-backend): 스캐너 고도화 테스트 후 삭제<br><br>**[정기진단 대상 현황]**<br>• ⬜ [01월] GWS/OB 20건: oki-admin-fe, oki-be×7, oki-fe / cashbagmall×4, front_resource, ob-backend×5<br>• ⬜ [02월] OCB_BACK_END/OCB-GAME/OCB-THP/OCBWEBVIEW/OKICK 19건 (유효, 2건 제외)<br>  - OCB_BACK_END: ocb-cashbag-mall×2 / OCB-GAME: ocb-game-admin, ocb-game-batch<br>  - OCB-THP: PHP 서비스 5건 / OCBWEBVIEW: ocb-community-api·ssr·ogeul-admin-fe·webview-admin-api<br>  - OKICK: okick-event-batch/server, okick-front, okick-reward-batch/front/server<br>• ⬜ [03월] OEP/OL/OTH/TALKS/OCBPASS 32건<br>  - OEP: event_resource, ob-promotion×2 / OL: locker-* 11건<br>  - OTH: homeshopping·trend-ad·trend-cms·trendissue / TALKS: oggletalk-*·talkplanet-* 8건<br>  - OCBPASS: ocbpass-11st·admin·app·batch·inside·newpg | 🔴 | L | 🔄 진행중 | 전체 파이프라인<br>`tools/scripts/` | 2026-03-13 | - | Fortify 1분기 OCB 진단 대상 목록 참조 (pageId=728911610) |
