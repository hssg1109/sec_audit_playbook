...
<!-- 아래 표의 심각도 셀(Critical/High)은 Confluence 게시 시 자동으로 컬러 배지로 변환됨 -->

| # | 취약점 유형 | 심각도 | 파일 | 라인 | 판정 |
|---|-----------|--------|------|------|------|
| 1 | <issue_name> | Critical | `파일명.java` | 164 | **취약** |
| 2 | <issue_name> | High | `파일명.java` | 219 | **취약** |
| ... | | | | | |

---

### 2-1. <취약점 유형> × N건 — Critical/High
...
□ 보고서 생성: state/<prefix>_ssc_report.md (아래 섹션 전체 포함)
   □ 섹션1: 요약 + 그룹별 검증 결과 표 (14그룹 또는 실제 그룹 수)
   □ 섹션2: 취약 확인 건 목록 표 (TP 1건=1행, 심각도/파일/라인 명시) ← 필수
   □ 섹션2 하위: 취약점 유형별 상세 (코드 증적 + 테인트 경로 + 조치 방안)
   □ 섹션3: 양호 판정 요약 (issue_name별 FP 근거)
   □ 섹션4: 추가 검토필요 → LLM 분석 결과
   □ 섹션5: 조치 우선순위
