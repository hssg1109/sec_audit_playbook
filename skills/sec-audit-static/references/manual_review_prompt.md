...
```json
{
  ...
  "diagnosis_detail": "SearchRequest.keyword(String) → ItemQuery.keyword → MyBatis ${keyword} 직접 삽입. DTO 래핑으로 스크립트 자동 추적 실패였으나 수동 역추적으로 취약 확인.",
  "diagnosis_method": "수동진단(LLM) — Taint 역추적",
  "taint_path": "Controller(@RequestBody) → Service(ItemQuery.keyword) → Mapper(${keyword})",
  "needs_review": false,
  "manual_review_note": "XML 코드 직접 확인 — 추측 없음"
}
```
