...
cat state/<prefix>_audit_memory.md
```

`state/<prefix>_audit_memory.md` 파일이 **비어있지 않으면**, 해당 내용이
...
- **`db_operations`가 비어있거나 `access_type`이 `unknown`인 항목** → 아래 Taint 역추적 진단 프롬프트 적용

---

## SQL Injection Taint 역추적 진단 프롬프트

### 적용 대상

스크립트 결과에서 아래 조건 중 하나라도 해당하는 항목:

```json
{ "db_operations": [] }
{ "db_operations": [{ "access_type": "unknown" }] }
{ "db_operations": [{ "access_type": "external_module", "needs_review": true }] }
```

이는 스크립트가 Controller → Service → Repository 호출 경로를 완전히 추적하지 못하여
DB 접근 방식을 자동 판정하지 못한 케이스입니다. 아래 역추적 절차에 따라 LLM이
직접 Taint 흐름을 연결합니다.
...
```
만약 스크립트 결과에 db_operations가 비어있거나 access_type이 unknown인 경우,
LLM은 제공된 Controller/Service 코드를 보고 아래 두 가지를 최우선으로 역추적하여
Taint 흐름을 직접 연결해야 합니다.

[역추적 우선순위 1] DTO/Map 파라미터 래핑 확인
  - HTTP 파라미터(Request Param / Request Body)가 DTO나 Map으로 묶여서
    Service 또는 Repository로 전달되는지 확인합니다.
  - 확인 방법:
    (a) Controller 메서드 파라미터 선언: @RequestParam, @RequestBody, @PathVariable
    (b) 해당 파라미터가 new SomeDto(), new HashMap() 등으로 래핑되는 지점 탐색
    (c) 래핑된 객체(dto, map, req 등)가 Service/Repository 호출 인자로 전달되는지 확인
    (d) Service 내부에서 dto.getXxx() / map.get("key") 형태로 꺼내 SQL에 전달되는지 추적
  - 판정:
    - DTO 내부의 String 타입 필드가 MyBatis ${} 또는 +문자열 결합에 전달 → 취약
    - DTO 내부의 String 타입 필드가 #{} 바인딩에 전달 → 양호
    - Integer/Long/Enum 타입 필드만 전달 → 양호 (타입 캐스팅으로 인젝션 차단)

[역추적 우선순위 2] iBatis/MyBatis SQL ID 동적 생성 확인
  - Service 또는 Repository에서 SQL ID가 런타임에 결정되는지 확인합니다.
  - 위험 패턴:
    (a) sqlSession.selectList("namespace." + dynamicId, param)
    (b) String sqlId = condition ? "findA" : "findB"; sqlSession.select(sqlId, ...)
    (c) XML <include refid="${dynamicFragment}"/> (EL 동적 fragment 참조)
  - 판정:
    - SQL ID 자체가 사용자 입력에 의해 변경될 수 있으면 → 취약 (SQL ID Injection)
    - SQL ID가 내부 상수/enum으로만 결정되면 → 안전, 실제 SQL 패턴으로 재판정
    - XML을 직접 확인할 수 없으면 → 정보 (needs_review: true 유지)

[역추적 후 판정 보고 형식]
  - 추적한 Taint 경로를 아래 형식으로 명시합니다:
    "Controller(@RequestParam String keyword)
     → Service(SearchDto dto = new SearchDto(keyword))
     → Repository(searchMapper.search(dto.getKeyword()))
     → MyBatis XML(${}|#{} 확인 필요)"
  - 경로 추적이 불가한 지점은 "→ [추적 불가: 이유]"로 표시합니다.
  - 추측이 포함된 경우 "추측:" 접두사를 붙여 명시합니다.
```
...
- API 경로 + HTTP Method: [예: POST /api/v1/items/search]
- 스크립트 판정: db_operations 비어있음 / access_type: unknown
- service_calls: [예: ItemService.searchItems()]
- repository_calls: [] (추적 실패)

### Controller 코드 (파라미터 선언 + 메서드 본문 전체)
```java
...
### Service 코드 (메서드 본문 전체)
```java
public List<ItemDto> searchItems(SearchRequest request) {
    ItemQuery query = ItemQuery.builder()
        .keyword(request.getKeyword())
        .category(request.getCategory())
        .build();
    return itemMapper.findItems(query);
}
```

### Repository / Mapper 코드 또는 XML (있는 경우)
```xml
<select id="findItems" parameterType="ItemQuery">
  SELECT * FROM items
...
public class SearchRequest {
    private String keyword;  // 사용자 자유 텍스트
    private String category; // 사용자 자유 텍스트
}
...
Controller(@RequestBody SearchRequest request)
  → request.getKeyword() [String, 사용자 자유 텍스트]
  → Service: ItemQuery.keyword = request.getKeyword()
  → Repository: itemMapper.findItems(query) → query.getKeyword()
  → MyBatis XML: ${keyword} — PreparedStatement 미사용, 직접 삽입 확인

**[역추적 1] DTO 래핑 확인:**
- SearchRequest → ItemQuery 변환 시 keyword 필드(String)가 보존됨
- ItemQuery.keyword가 MyBatis ${keyword}에 전달됨 → 취약

**[역추적 2] SQL ID 동적 생성:**
- SQL ID "findItems"는 정적 상수, 동적 생성 없음

**판정 근거:**
- keyword: String 타입 사용자 입력 → ${keyword} 직접 삽입 → SQL Injection 가능
- OWASP A03:2021 / KISA DBMS 조회 및 결과 검증 취약점

**불확실 사항:** 없음 (XML 직접 확인됨)
...
> **[Project Specific Context & Exceptions]가 제공된 경우**, 이는 해당 프로젝트의
> 보안 담당자가 코드 검토를 통해 **확정한 절대적인 오탐(FP) 기준**입니다.
>
> - 스캐너가 취약점으로 분류했더라도, 분석 대상 항목의 파일 경로가
>   `target_class_or_path`에 **부분 일치**하고 `vulnerability_type`이 일치하면
>   → **즉시 `양호(FP)`로 판정**하십시오.
> - 추가 코드 분석 없이 확정합니다. 예외 규칙이 이미 보안 담당자의 코드 검토를 거쳤기 때문입니다.
> - 판정 결과의 `reason` 필드에 **FP 규칙 ID와 `reason` 내용을 그대로** 기재합니다.
>   예: `"reason": "[FP-001] SafeQueryBuilder — _validateInput() 화이트리스트 선행 검증 확정"`
> - FP 규칙과 **일치하지 않는** 항목은 아래 [진단 기준]에 따라 정상 분석합니다.
...
[Step 0] 순수 API 여부 교차 검증 (필수 선행 단계)

분석 대상 메서드가 실제로 HTML View를 렌더링하는지 먼저 확인하십시오.
클래스 레벨에 @Controller가 붙어 있더라도 아래 조건 중 하나라도 해당되면
해당 메서드는 HTML View를 렌더링하지 않는 '순수 API'입니다.
즉시 [양호(순수 API 데이터 반환)]으로 판정하고 이후 점검 항목(1~4)을 생략하십시오.

  [조건 A] 메서드에 @ResponseBody 어노테이션이 선언되어 있는 경우
           → Content-Type이 application/json으로 반환되므로 브라우저가 HTML로 해석 불가

  [조건 B] 반환 타입이 ResponseEntity<T> 인 경우 (T 타입 불문)
           → Spring이 HTTP 응답 객체를 직렬화하여 반환; View 렌더링 경로 없음

  [조건 C] 반환 타입이 Protobuf 생성 클래스인 경우
           (예: MainProtos.GetMainBlockResponse, FooProtos.SomeMessage,
            com.google.protobuf.Message 파생 클래스 등)
           → 바이너리/JSON 직렬화 응답; HTML 렌더링과 무관

  [조건 D] 반환 타입이 위 외에도 순수 데이터 객체(DTO, Map, List 등)이며
           @RequestMapping(produces = "application/json") 또는 produces = MediaType.APPLICATION_JSON_VALUE
           가 메서드 또는 클래스 레벨에 명시된 경우

위 조건에 해당하지 않는 경우에만 아래 점검 항목을 계속 진행하십시오.
